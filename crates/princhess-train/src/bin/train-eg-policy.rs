...
fn run_training_loop<S: LRScheduler>(
    ...
    mut optimizer: AdamWOptimizer<EgPolicyNetwork, S>,
...
fn train_super_batch<S: LRScheduler>(
    ...
    optimizer: &mut AdamWOptimizer<EgPolicyNetwork, S>,
    config: &TrainingConfig,
    stats: &TrainingStats,
...
fn gradients_batch(
    ...
    // We accept this for performance: in practice on x86-64, sparse-weight rows
    // rarely collide, and gradient noise in the dense output layer is negligible
    // for SGD convergence.
    let gradients_ptr = gradients as *mut EgPolicyNetwork as usize;
    ...
    let metrics: Vec<BatchMetrics> = thread::scope(|s| {
        batch
            ...
            .map(|chunk| {
                s.spawn(move || {
                    let grads = unsafe { &mut *(gradients_ptr as *mut EgPolicyNetwork) };
