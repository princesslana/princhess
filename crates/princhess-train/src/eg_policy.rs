...
use crate::nets;
use crate::neural::{
    AsParams, FeedForwardNetwork, Optimizable, OutputLayer, PiecewiseTanh, SparseConnected,
...
const _: () = {
    assert!(std::mem::size_of::<EgPolicyNetwork>() % std::mem::size_of::<f32>() == 0);
...
// SAFETY: EgPolicyNetwork is #[repr(C)], composed entirely of f32 with no padding.
unsafe impl Optimizable for EgPolicyNetwork {}
