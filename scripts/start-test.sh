...
# Usage: start-test.sh <test_type> <time_control> <engine1> <engine2>
...
echo "Generating command for $TEST_TYPE test ($TIME_CONTROL): $ENGINE1 vs $ENGINE2"
FASTCHESS_ARGS=$("$SCRIPT_DIR/generate-args.sh" "$TEST_TYPE" "$TIME_CONTROL" "$ENGINE1" "$ENGINE2")
